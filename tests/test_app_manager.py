...
class TestLaunch:
    def _launch(self, am, idx=0, command="echo", args=None, pid=1234):
        ...
        with patch("infrastructure.kde.catalog.app_manager.subprocess.Popen", return_value=proc) as popen, \
    ...
    def test_ignored_when_same_idx_already_running(self, qapp):
        am = _make_manager()
        am._processes[0] = _running_proc()
        with patch("infrastructure.kde.catalog.app_manager.subprocess.Popen") as popen:
            am.launch(0, "echo")
    ...
    def test_starts_monitor_thread(self, qapp):
        am = _make_manager()
        proc = _running_proc()
        with patch("infrastructure.kde.catalog.app_manager.subprocess.Popen", return_value=proc), \
    ...
    def test_returns_false_when_already_running(self, qapp):
        am = _make_manager()
        am._processes[0] = _running_proc()
        with patch("infrastructure.kde.catalog.app_manager.subprocess.Popen"):
    ...
    def test_returns_false_and_emits_failed_on_missing_command(self, qapp):
        ...
        with patch("infrastructure.kde.catalog.app_manager.subprocess.Popen", side_effect=FileNotFoundError):
    ...
    def test_returns_false_on_permission_error(self, qapp):
        am = _make_manager()
        with patch("infrastructure.kde.catalog.app_manager.subprocess.Popen", side_effect=PermissionError):
...
class TestTerminate:
    ...
    def test_sends_sigterm(self, qapp):
        ...
        with patch("infrastructure.kde.catalog.app_manager.os.getpgid", return_value=1234), \
             patch("infrastructure.kde.catalog.app_manager.os.killpg") as mock_killpg, \
    ...
    def test_schedules_force_kill_after_3s(self, qapp):
        am = _make_manager()
        am._processes[0] = _running_proc()
        with patch("infrastructure.kde.catalog.app_manager.os.getpgid", return_value=999), \
             patch("infrastructure.kde.catalog.app_manager.os.killpg"), \
    ...
    def test_noop_when_process_already_exited(self, qapp):
        am = _make_manager()
        am._processes[0] = _exited_proc()
        with patch("infrastructure.kde.catalog.app_manager.os.killpg") as mock_killpg:
            am.terminate(0)
    ...
    def test_terminate_only_affects_target_idx(self, qapp):
        ...
        am._processes[0] = _running_proc(pid=100)
        am._processes[1] = _running_proc(pid=200)
        with patch("infrastructure.kde.catalog.app_manager.os.getpgid", return_value=100), \
             patch("infrastructure.kde.catalog.app_manager.os.killpg") as mock_killpg, \
             patch("infrastructure.common.lifecycle.base_app_manager.QTimer.singleShot"):
            am.terminate(0)
...
class TestForceKill:
    def test_sends_sigkill_when_still_running(self, qapp):
        ...
        with patch("infrastructure.kde.catalog.app_manager.os.getpgid", return_value=5678), \
             patch("infrastructure.kde.catalog.app_manager.os.killpg") as mock_killpg:
            am._force_kill(proc)
    ...
    def test_noop_when_process_no_longer_tracked(self, qapp):
        ...
        am._processes[0] = new
        with patch("infrastructure.kde.catalog.app_manager.os.getpgid", return_value=1111), \
             patch("infrastructure.kde.catalog.app_manager.os.killpg") as mock_killpg:
