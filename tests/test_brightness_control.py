...
from infrastructure.kde.display.brightness import (
...
class TestBrightnessctlGet:
    def test_parses_percent(self):
        ...
        with patch("infrastructure.kde.display.brightness.subprocess.check_output", return_value=out):
            assert BrightnessctlBrightnessControl().get().value == 40

    def test_default_on_error(self):
        with patch("infrastructure.kde.display.brightness.subprocess.check_output", side_effect=FileNotFoundError):
...
class TestBrightnessctlSet:
    def test_calls_brightnessctl_with_percent(self):
        with patch("infrastructure.kde.display.brightness.subprocess.Popen") as popen:
    ...
    def test_swallows_errors(self):
        with patch("infrastructure.kde.display.brightness.subprocess.Popen", side_effect=OSError):
...
class TestKdeBrightnessControl:
    ...
    def test_set_scales_percent_to_absolute(self):
        with patch.object(KdeBrightnessControl, "_call", return_value="1000"), \
             patch("infrastructure.kde.display.brightness.subprocess.Popen") as popen:
...
class TestSelector:
    def test_prefers_brightnessctl(self):
        with patch("infrastructure.kde.display.brightness.shutil.which", return_value="/usr/bin/brightnessctl"):
    ...
    def test_falls_back_to_kde_with_qdbus6(self):
        ...
        with patch("infrastructure.kde.display.brightness.shutil.which", side_effect=which):
    ...
    def test_falls_back_to_null(self):
        with patch("infrastructure.kde.display.brightness.shutil.which", return_value=None):
