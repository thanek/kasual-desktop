...
class TestGet:
    def test_parses_percent(self):
        ...
        with patch("infrastructure.linux.audio.volume.subprocess.check_output", return_value=out):
...
class TestSet:
    def test_calls_pactl_with_percent(self):
        with patch("infrastructure.linux.audio.volume.subprocess.Popen") as popen:
    ...
    def test_swallows_errors(self):
        with patch("infrastructure.linux.audio.volume.subprocess.Popen", side_effect=OSError):
