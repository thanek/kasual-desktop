...
class TestGet:
    def test_parses_percent(self):
        ...
        with patch("infrastructure.kde.audio.volume.subprocess.check_output", return_value=out):
            assert PactlVolumeControl().get().value == 80

    def test_default_on_error(self):
        with patch("infrastructure.kde.audio.volume.subprocess.check_output", side_effect=FileNotFoundError):
            assert PactlVolumeControl().get() == Volume(Volume.DEFAULT)


class TestSet:
    def test_calls_pactl_with_percent(self):
        with patch("infrastructure.kde.audio.volume.subprocess.Popen") as popen:
    ...
    def test_swallows_errors(self):
        with patch("infrastructure.kde.audio.volume.subprocess.Popen", side_effect=OSError):
