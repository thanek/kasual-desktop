...
class TestSuspend:
    def test_calls_set_suspend_state_with_false_false_false(self):
        with patch("infrastructure.windows.power.power.ctypes.windll") as windll:
...
class TestReboot:
    def test_calls_shutdown_with_r_flag(self):
        with patch("infrastructure.windows.power.power.subprocess.run") as run:
    ...
    def test_swallows_exception(self):
        with patch("infrastructure.windows.power.power.subprocess.run",
                   side_effect=OSError):
